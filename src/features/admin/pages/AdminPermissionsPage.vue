<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col gap-4">
      <div>
        <h2 class="font-anton text-xl text-gray-900">Permissions</h2>
        <p class="text-sm text-gray-500 font-love mt-1">
          <span class="inline-flex items-center gap-1.5">
            <span class="h-2 w-2 rounded-full bg-secondary"></span>
            {{ adminStore.permissionsTotal }} permission(s) au total
          </span>
        </p>
      </div>
      <button @click="openCreateModal" class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-secondary text-white text-sm font-love font-medium hover:bg-secondary/80 active:scale-95 transition-all shadow-sm shadow-secondary/20">
    ...
    <!-- Cards -->
    <div v-else-if="adminStore.permissions.length > 0" class="space-y-3">
      <div
        v-for="p in adminStore.permissions"
        :key="p.id"
        ...
        <div class="p-4">
          <div class="flex items-start gap-3 mb-3">
            <div class="h-10 w-10 rounded-xl bg-secondary/10 flex items-center justify-center flex-shrink-0">
              <i class="ri ri-shield-line text-secondary" style="font-size: 18px;"></i>
            </div>
          ...
          <div class="flex items-center gap-2">
            <button @click="openEditModal(p)" class="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-love font-semibold text-secondary border border-secondary/10 hover:bg-secondary/10 active:scale-95 transition-all">
              Modifier
            </button>
      ...
      <div class="flex items-center justify-center pt-2">
        <button @click="loadMore" :disabled="adminStore.permissions.length < limit" class="inline-flex items-center gap-2 px-6 py-2.5 rounded-xl text-sm font-love font-semibold text-secondary bg-white border border-primary/10 hover:bg-secondary/10 disabled:opacity-40 disabled:cursor-not-allowed transition-all">
          <i class="ri ri-add-line" style="font-size: 16px;"></i>
          Voir plus
        </button>
      </div>
    ...
    <div v-else class="flex flex-col items-center justify-center py-12 px-4 bg-gray-50 rounded-3xl">
      <div class="h-16 w-16 rounded-full bg-secondary/10 flex items-center justify-center mb-4">
        <i class="ri ri-shield-line text-secondary/50" style="font-size: 32px;"></i>
      </div>
      <p class="text-sm text-gray-900 font-love font-semibold">Aucune permission trouvée</p>
      <p class="text-xs text-gray-500 font-love mt-1">Créez votre première permission</p>
    </div>
    ...
    <!-- Drawer -->
    <Drawer :show="showModal" :title="editingId ? 'Modifier' : 'Nouvelle' + ' permission'" maxHeight="80vh" @close="closeModal">
      <form @submit.prevent="handleSubmit" class="space-y-4">
        ...
        <div>
          <label class="block text-sm font-love font-medium text-gray-700 mb-1.5">Slug</label>
          <input v-model="form.slug" type="text" required placeholder="Ex: admin.users.read" class="w-full text-sm font-love border border-primary/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/10 transition-all" />
        </div>
        <div>
          <label class="block text-sm font-love font-medium text-gray-700 mb-1.5">Module</label>
          <input v-model="form.module" type="text" placeholder="Ex: admin, chat, payment" class="w-full text-sm font-love border border-primary/10 rounded-xl px-4 py-2.5 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/10 transition-all" />
        </div>
        <div>
...
const form = reactive({ name: '', slug: '', description: '', module: '' });

const openCreateModal = () => { editingId.value = null; Object.assign(form, { name: '', slug: '', description: '', module: '' }); showModal.value = true; };
const openEditModal = (p: any) => { editingId.value = p.id; Object.assign(form, { name: p.name, slug: p.slug, description: p.description || '', module: p.module || '' }); showModal.value = true; };
const closeModal = () => { showModal.value = false; editingId.value = null; };

const handleSubmit = async () => {
  try {
    if (editingId.value) { await adminStore.updatePermission(editingId.value, { ...form }); }
    else { await adminStore.createPermission({ ...form }); }
    closeModal();
  } catch { /* error in store */ }
};

const handleDelete = (id: number) => {
  requestConfirm('Voulez-vous vraiment supprimer cette permission ?', async () => {
    try { await adminStore.removePermission(id); } catch { /* error in store */ }
...
const loadMore = () => { currentPage.value++; adminStore.fetchPermissions(currentPage.value, limit); };

onMounted(() => { adminStore.fetchPermissions(currentPage.value, limit); });
</script>
