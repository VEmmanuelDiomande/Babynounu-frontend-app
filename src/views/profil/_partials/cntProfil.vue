<template>
  <IonPage>
    ...
    >
      <IonToolbar
        class="border-b-[1px] shadow-none w-full px-2 m-auto flex justify-between"
      >
      ...
      <section class="w-11/12 m-auto py-4">
        <div class="flex flex-col w-full">
          ...
          <div class="flex flex-row justify-between gap-4">
            ...
            <div class="flex bg-zinc-400 size-16 rounded-full overflow-hidden">
              <img
                v-lazy="Data.image?.originalUrl || '/assets/default-avatar.png'"
          ...
          <div class="bg-white rounded-lg shadow-sm p-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="border rounded-lg p-3">
                ...
                <div class="text-xl font-bold">{{ statsData.responseRate || '0%' }}</div>
              </div>
              <div class="border rounded-lg p-3">
                <div class="text-sm text-gray-500">Temps de réponse</div>
                <div class="text-xl font-bold">{{ statsData.responseTime || '0h' }}</div>
              </div>
              <div class="border rounded-lg p-3">
                <div class="text-sm text-gray-500">Missions terminées</div>
                <div class="text-xl font-bold">{{ statsData.completedMissions || 0 }}</div>
              </div>
              <div class="border rounded-lg p-3">
                <div class="text-sm text-gray-500">Heures travaillées</div>
                <div class="text-xl font-bold">{{ statsData.hoursWorked || 0 }}h</div>
              </div>
            </div>
          </div>
        </section>
...
const shouldShowStatusAlert = computed(() => {
  return (props.Data.certif === 'Rejected' || props.Data.certif === 'Pending') && 
...
const getStatusTitle = computed(() => {
  return props.Data.certif === 'Rejected' ? 'Profil rejeté' : 'Profil en attente';
});
