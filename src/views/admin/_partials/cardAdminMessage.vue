<template>
          ...
          >
            <div class="mt-4 flex flex-row gap-4">
              ...
              <div class="relative flex w-2/12">
                  <img
                    v-lazy="message?.parentPhoto?.originalUrl"
                    class="rounded-full size-10 absolute"
                    alt=""
                  />
                  <img
                    v-lazy="message?.nounuPhoto?.originalUrl"
              ...
              <div class="w-[calc(100%-70px)] flex flex-col gap-1">
                ...
                <div class="flex gap-2 items-center w-full">
                  ...
                  <span class="truncate w-full font-medium text-black text-base">
                    {{  message.lastMessage.content.includes('{"price') ? 'Proposition de service' : message.lastMessage.content }}
                  </span>
                  <div
  ...
  const formattedMessages = computed(() => {
    return Messages
      ...
      .sort((a, b) =>
        moment(b.room.lastMessage?.createdAt).unix() -
        moment(a.room.lastMessage?.createdAt).unix()
      )
      .map((msg) => {
        return {
          ...msg.room,
